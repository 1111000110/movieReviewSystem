...
type (
	...
	Users struct {
		...
		BirthDate  sql.NullInt64  `db:"birth_date"`
		Role       sql.NullString `db:"role"`
		Status     sql.NullInt64  `db:"status"`
		CreateDate sql.NullInt64  `db:"create_date"`
		UpdataDate sql.NullInt64  `db:"updata_date"`
	}
)
...
func (m *defaultUsersModel) Insert(ctx context.Context, data *Users) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.Phone, data.Email, data.Password, data.NickName, data.Avatar, data.Gender, data.BirthDate, data.Role, data.Status, data.CreateDate, data.UpdataDate)
	return ret, err
}

func (m *defaultUsersModel) Update(ctx context.Context, newData *Users) error {
	query := fmt.Sprintf("update %s set %s where `user_id` = ?", m.table, usersRowsWithPlaceHolder)
	_, err := m.conn.ExecCtx(ctx, query, newData.Phone, newData.Email, newData.Password, newData.NickName, newData.Avatar, newData.Gender, newData.BirthDate, newData.Role, newData.Status, newData.CreateDate, newData.UpdataDate, newData.UserId)
	return err
}
